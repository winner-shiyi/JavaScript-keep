...
window.onload=function ()
{
	...
	for(i=0;i<aImg.length;i++)
	{
		...
		{
			g_aImg.push(aImg[i]);//所有的alt=0的图片集合
		}
	}
	g_aImg.push(aImg[aImg.length-2]);//alt=one的图片
	
	aNow=getTimeArray();//[2, 0, 1, 7, 0, 3, 1, 9, 1, 8, 2, 0, 3, 7, 6]
	
	console.log(g_aImg.length);//15
...
function checkSwitch()
{
	...
	timer=setInterval(doSwitch, 100);
}
...
function doSwitch()
{
	...
	for(i=0;i<g_aImg.length;i++)
	{
		if(g_aImg[i].now!=aNow[i])//[2, 0, 1, 7, 0, 3, 1, 9, 1, 8, 2, 0, 3, 7, 6]
		{
			if(g_imgHeigth>0)
			{
			...
			{
				if(i==g_aImg.length-1)//最后一张alt星期几的图片
				{
